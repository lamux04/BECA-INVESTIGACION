...
def calcular_roc_auc_multiclase_seguro(modelo, X_val, y_val, labels_globales):
    """
    Calcula ROC AUC multiclase alineando las probabilidades con todas las clases globales.
    Si no se puede calcular, devuelve np.nan.
    """

    try:
        y_proba = modelo.predict_proba(X_val)

        proba_alineada = np.zeros((len(X_val), len(labels_globales)))

        for i, clase in enumerate(modelo.classes_):
            idx_global = list(labels_globales).index(clase)
            proba_alineada[:, idx_global] = y_proba[:, i]
        ...
        print("No se pudo calcular ROC AUC en este fold:", e)
        return np.nan