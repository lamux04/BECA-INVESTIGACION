"""Agrega las metricas de test obtenidas en los cinco folds de la fase C."""
...
RUTA_RESULTADOS_FASE = (
    PROJECT_ROOT / "05_results" / DATASET_ORIGINAL / "fase_C_t5"
)
...
FOLD_RE = re.compile(r"C__t5_fold_(\d+)")
...
def _localizar_reportes_metricas():
    ...
    rutas = sorted(
        RUTA_RESULTADOS_FASE.glob(
            "C__t5_fold_*/reports/*__test_metrics.csv"
        )
    )
    reportes_por_fold = {}
...
def main():
    ...
    ruta_folds = (
        ...
        / f"{DATASET_ORIGINAL}__C_t5__test_metrics_by_fold.csv"
    )
    ruta_resumen = (
        RUTA_SALIDA_COMPARACION
        / f"{DATASET_ORIGINAL}__C_t5__test_metrics_summary.csv"
    )

    df_folds.to_csv(ruta_folds, index=False)
    df_resumen.to_csv(ruta_resumen, index=False)

    print("Comparativa de la fase C generada correctamente.")
