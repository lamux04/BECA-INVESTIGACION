"""Agrega las metricas de test obtenidas en los cinco folds de la fase B."""
...
RUTA_RESULTADOS_FASE = (
    PROJECT_ROOT / "05_results" / DATASET_ORIGINAL / "fase_B_t5"
)
...
FOLD_RE = re.compile(r"B__t5_fold_(\d+)")


def _extraer_fold(ruta_reporte):
    """Obtiene el numero de fold a partir del directorio del experimento."""
    match = FOLD_RE.fullmatch(ruta_reporte.parents[1].name)

    if match is None:
        raise ValueError(
            f"No se pudo extraer el fold desde: {ruta_reporte}"
        )
...
def _localizar_reportes_metricas():
    """Localiza y valida un unico reporte de metricas para cada fold."""
    rutas = sorted(
        RUTA_RESULTADOS_FASE.glob(
            "B__t5_fold_*/reports/*__test_metrics.csv"
        )
    )
...
def _cargar_resultados_folds(reportes_por_fold):
    """Construye una tabla con una fila de metricas por fold."""
    filas = []
...
def _calcular_resumen_metricas(df_folds):
    """Calcula un resumen horizontal con media y desviacion por metrica."""
    resumen = {
        "dataset": DATASET_ORIGINAL,
        "phase": "B_t5",
...
def main():
    ...
    ruta_folds = (
        ...
        / f"{DATASET_ORIGINAL}__B_t5__test_metrics_by_fold.csv"
    )
    ruta_resumen = (
        RUTA_SALIDA_COMPARACION
        / f"{DATASET_ORIGINAL}__B_t5__test_metrics_summary.csv"
    )

    df_folds.to_csv(ruta_folds, index=False)
    df_resumen.to_csv(ruta_resumen, index=False)

    print("Comparativa de la fase B generada correctamente.")
