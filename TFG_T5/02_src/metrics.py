...
def calcular_metricas_desde_predicciones(
    y_true,
    y_pred,
    labels_globales,
    score_time,
):
    """Calcula metricas para modelos que no exponen probabilidades."""
    y_true = pd.Series(y_true).astype(str).reset_index(drop=True)
    y_pred = pd.Series(y_pred).astype(str).reset_index(drop=True)
    labels_globales = np.asarray([str(label) for label in labels_globales])
    ...
    metricas = {
        ...
        "detection_latency_seconds": float(score_time / len(y_true))
        if len(y_true) > 0
        else 0.0,
        "score_time": float(score_time),
    }
    labels_inesperadas = sorted(set(y_pred).difference(labels_globales))
    labels_matriz = np.concatenate([labels_globales, labels_inesperadas])
    cm = confusion_matrix(y_true, y_pred, labels=labels_matriz)
    total = cm.sum()
    soporte = cm.sum(axis=1)[:len(labels_globales)].astype(float)
    fpr_por_clase = []
    fnr_por_clase = []

    for index in range(len(labels_globales)):
        tp = cm[index, index]
        fp = cm[:, index].sum() - tp
        fn = cm[index, :].sum() - tp
        tn = total - tp - fp - fn
        fpr_por_clase.append(fp / (fp + tn) if (fp + tn) > 0 else 0.0)
        fnr_por_clase.append(fn / (fn + tp) if (fn + tp) > 0 else 0.0)

    metricas.update(
        {
            "fpr_macro": float(np.mean(fpr_por_clase)),
            "fnr_macro": float(np.mean(fnr_por_clase)),
            "fpr_weighted": float(np.average(fpr_por_clase, weights=soporte)),
            "fnr_weighted": float(np.average(fnr_por_clase, weights=soporte)),
        }
    )
