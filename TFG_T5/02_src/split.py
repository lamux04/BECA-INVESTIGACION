...
def iterar_folds_stratified(df, label_col="LABEL", n_splits=5, random_state=42):
    """Itera sobre pares train/validacion estratificados de un dataset."""
    if label_col not in df.columns:
        raise KeyError(f"La columna de etiquetas no existe: {label_col}")
    ...
    class_counts = df[label_col].value_counts()
    clases_insuficientes = class_counts[class_counts < n_splits]

    if not clases_insuficientes.empty:
        clases = ", ".join(
            f"{label} ({count})"
            for label, count in clases_insuficientes.items()
        )
        raise ValueError(
            f"Cada clase necesita al menos {n_splits} muestras para generar "
            f"los folds. Clases afectadas: {clases}"
        )
    ...
    for fold, (train_indices, val_indices) in enumerate(
        ...
        fold_train_df = df.iloc[train_indices].reset_index(drop=True)
        fold_val_df = df.iloc[val_indices].reset_index(drop=True)
