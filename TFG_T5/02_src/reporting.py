...
def registrar_parametros_split(
    test_size,
    random_state,
    label_col=None,
    n_splits=None,
    ...
    if n_splits is not None:
        registro["n_splits"] = n_splits

    _context_list("split_parameters").append(registro)


def registrar_resumen_split(
    df_original,
    train_df,
    test_df,
):
...
def registrar_particion_split(
    df_partition,
    partition,
    reference_rows,
    label_col=None,
):
    """Registra el resumen y la distribucion de una particion adicional."""
    rows = len(df_partition)
    ...
    _context_list("split_summary").append(
        {
            ...
            "rows": rows,
            "columns": len(df_partition.columns),
            "proportion": rows / reference_rows if reference_rows > 0 else 0,
        }
    )
        ...
        _context_list("split_label_distribution").append(
            {
                ...
                "label": label,
                "count": int(count),
                "proportion": count / rows if rows > 0 else 0,
            }
        )
