...
############### MEDOIDES ###################
class KMedoidsPerClassClassifier(BaseEstimator, ClassifierMixin):
    def __init__(
        self,
        k_medoids_per_class=5,
        metric="euclidean",
        random_state=42,
        distance_to_proba_eps=1e-9,
        max_samples_per_class=10000
    ):
        self.k_medoids_per_class = k_medoids_per_class
        self.metric = metric
        self.random_state = random_state
        self.distance_to_proba_eps = distance_to_proba_eps
    ...
    def fit(self, X, y):
        ...
        for cls in self.classes_:
            ...
            if len(X_cls) > self.max_samples_per_class:
                indices = rng.choice(
                    len(X_cls),
                    size=self.max_samples_per_class,
                    replace=False
                )
                X_cls_fit = X_cls[indices]
            else:
                X_cls_fit = X_cls

            k = min(self.k_medoids_per_class, len(X_cls_fit))
            ...
            self.medoids_.append(model.cluster_centers_)
            self.medoid_labels_.extend([cls] * k)

        self.medoids_ = np.vstack(self.medoids_)
        self.medoid_labels_ = np.array(self.medoid_labels_)

        return self

    def predict(self, X):
        check_is_fitted(self, ["medoids_", "medoid_labels_", "classes_"])
        ...
        nearest_medoid_idx = np.argmin(distances, axis=1)

        return self.medoid_labels_[nearest_medoid_idx]
    ...
    def predict_proba(self, X):
        ...
        for i, cls in enumerate(self.classes_):
            mask_cls = self.medoid_labels_ == cls
            min_dist_cls = distances[:, mask_cls].min(axis=1)
            score_cls = 1 / (min_dist_cls + self.distance_to_proba_eps)
            proba[:, i] = score_cls
